/**
 * qutip_dashboard.js — UI controller for the QuTiP-engine coincidence
 * dashboard. Twin of numpy_dashboard.js: draws the analytic closed-form curve
 * (instant) and overlays a live truncated-Fock simulation at fixed N=60
 * (open-circle markers) plus a log-scale residual break-off panel.
 *
 * The browser cannot run QuTiP itself, so the live overlay is computed by the
 * shared array engine in spdc_physics.js (SPDC.fockCoincidence). That engine
 * is the validated numerical twin of spdc.fock_qutip: at every benchmark point
 * the two agree to the Fock-truncation floor (~1e-11), so the markers you drag
 * here are, to machine precision, the same numbers QuTiP produces. The static
 * default render on this page IS computed by real QuTiP (spdc.fock_qutip).
 *
...
function updatePlot() {
    ...
    // trace 0: closed line, trace 1: QuTiP/Fock markers (both top),
