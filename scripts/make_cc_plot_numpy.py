"""
Generate the NumPy-engine coincidence dashboard iframe.

This is the NumPy twin of ``make_cc_plot.py``. On top of the closed-form
coincidence curve it overlays a *live* truncated-Fock ("brute force")
simulation at fixed Hilbert dimension N=60: open-circle markers sampled at
a handful of HWP angles, plus a log-scale residual break-off panel showing
|numeric - closed| at those angles. The pedagogical point: as lambda grows
the mean photon number climbs past what N=60 Fock states can represent, so
the numerical markers lift off the (truncation-free) closed form. Both the
closed curve (instant analytic JS) and the Fock markers (spdc_physics.js
fockCoincidence, browser twin of spdc.fock_numpy) recompute on every drag
tick from the shared window.SPDC module.

Physics is imported from the shared ``spdc`` package for the static default
render; the in-browser recompute lives in docs/js/numpy_dashboard.js.
"""
...
from spdccc.fock_numpy import brute_Pcoinc

# Fixed Fock-space parameters (mirrored in numpy_dashboard.js).
N_FOCK = 60
NMAX_FOCK = 45
N_MARKERS = 15
...
output_file = output_dir / "coincidence_plot_numpy.html"
...
muted = "#aeb4c2"
grid = "#3a4050"
amber = "#ff9f43"  # Fock markers / residual — reads as "the numeric one"
...
# --- top: closed-form curve + live Fock markers ------------------------
fig.add_trace(
    go.Scatter(
        x=theta,
        y=y,
...
fig.add_trace(
    go.Scatter(
        ...
        name=f"Fock N={N_FOCK}",
        hovertemplate="ϑ=%{x:.4f}<br>C/Cmax=%{y:.4f}<extra>Fock N=60</extra>",
    ),
    row=1,
    col=1,
)
...
<style>
  /* Page-scoped override: the shared coincidence_plots.css is sized for the
     ...
     more room, so we relax the clamps for THIS page only. The QuTiP page keeps
     the shared single-panel sizing. */
...
<script src="../../js/numpy_dashboard.js"></script>
"""
